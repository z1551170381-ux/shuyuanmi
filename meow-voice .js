...
` : ' ';
  ...
  function _buildBgmCaption(cArg, group, track, tracks) {
    const c = cArg || cfg();
    const groupName = String(group?.name || '未分组').trim() || '未分组';
    const titleRaw = String(track?.title || _bgmState.title || c.bgmTitle || '背景音乐').trim() || '背景音乐';
    const bits = titleRaw.split(/\s*[｜|/·]|\s+-\s+|\s+—\s+/).map(s => String(s || '').trim()).filter(Boolean);
    const mainTitle = bits[0] || titleRaw;
    const subTitle = bits.slice(1).join(' · ');
    const sourceKind = _bgmState.parsedKind || _parseDramaBgmSource(track?.url || _bgmState.sourceUrl || c.bgmUrl || '').kind || '';
    const sourceLabel = _bgmKindLabel(sourceKind);
    const lines = [];
  ...
  function _isBgmMobile() {
    return (W.innerWidth || doc.documentElement.clientWidth || 9999) <= 640;
  }

  /** 将 dock 定位到保存的位置（仅手机），否则恢复 CSS 默认锚点 */
  function _applyBgmDockMobilePos(root) {
    if (!root) return;
    if (!_isBgmMobile()) {
      // 桌面：恢复 CSS 控制，清除可能残留的 JS 内联定位
      root.style.left   = '';
      root.style.top    = '';
    ...
    const vw = W.innerWidth || 375;
    const vh = W.innerHeight || doc.documentElement.clientHeight || 667;
    const saved = lsGet(LS.BGM_DOCK_POS, null);
    // 默认：吸右边，高度 55%
    const x = saved?.x ?? (vw - 100);
    const y = saved?.y ?? Math.round(vh * 0.55);
    root.style.left   = Math.max(4, Math.min(vw - 96, x)) + 'px';
    root.style.top    = Math.max(48, Math.min(vh - 100, y)) + 'px';
    root.style.right  = 'auto';
    root.style.bottom = 'auto';
  }

  /** 吸附到最近的左/右边缘 */
  function _snapBgmDockToEdge(root) {
    if (!root || !_isBgmMobile()) return;
    const vw = W.innerWidth || 375;
    const x  = parseFloat(root.style.left) || 0;
    // 以 dock 中心判断靠哪侧
    const snapX = (x + 46) < vw / 2 ? 4 : vw - 96;
    root.style.left = snapX + 'px';
    lsSet(LS.BGM_DOCK_POS, { x: snapX, y: parseFloat(root.style.top) || 0 });
  }

  /** 为 dock 绑定拖拽逻辑（仅绑一次） */
  function _setupBgmDockDrag(root) {
    if (!root || root._mvDragBound) return;
    root._mvDragBound = true;

    let dragging = false, moved = false;
    let sx = 0, sy = 0, bx = 0, by = 0;

    const discWrap = root.querySelector('.mv-bgm-disc-wrap');
    if (!discWrap) return;

    const onStart = (e) => {
      if (!_isBgmMobile()) return;
    ...
    const onMove = (e) => {
      if (!dragging || !_isBgmMobile()) return;
      const p = e.touches ? e.touches[0] : e;
      ...
      const vw = W.innerWidth || 375;
      const vh = W.innerHeight || doc.documentElement.clientHeight || 667;
      root.style.left   = Math.max(4, Math.min(vw - 96, bx + dx)) + 'px';
      root.style.top    = Math.max(48, Math.min(vh - 100, by + dy)) + 'px';
      root.style.right  = 'auto';
      root.style.bottom = 'auto';
      e.preventDefault();
    };
    ...
    const onEnd = (e) => {
      if (!dragging) return;
      dragging = false;
      if (moved && _isBgmMobile()) {
        _snapBgmDockToEdge(root);
        e.preventDefault();
      }
      moved = false;
    };

    discWrap.addEventListener('touchstart', onStart, { passive: true });
    discWrap.addEventListener('mousedown',  onStart, { passive: false });
    ...
    // 重新定向：窗口尺寸改变时重新应用定位
    W.addEventListener('resize', () => _applyBgmDockMobilePos(root));
  }
  ...
  function _getBgmDock() {
    ...
    style.textContent = `
      ...
      #meow-voice-bgm-dock .mv-bgm-disc-ring{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:36px;height:36px;border-radius:50%;background:rgba(247,247,244,.94);box-shadow:inset 0 0 0 1px rgba(70,82,90,.10)}
      ...
      #meow-voice-bgm-dock.playing.collapsed .mv-bgm-tonearm{transform:rotate(-22deg)}
      #meow-voice-bgm-dock.collapsed:not(.playing) .mv-bgm-tonearm{transform:rotate(8deg)}
      ...
      #meow-voice-bgm-dock .mv-bgm-controls{display:flex;align-items:center;justify-content:center;gap:10px;margin-top:9px}
      #meow-voice-bgm-dock .mv-bgm-controls button{width:32px;height:32px;font-size:14px;line-height:1;border:0;border-radius:999px;background:rgba(255,255,255,.76);box-shadow:0 5px 14px rgba(16,50,55,.08);cursor:pointer;color:#3e4f56}
      ...
      #meow-voice-bgm-dock.collapsed{transform:translate3d(calc(100% - 92px),0,0)}
      ...
      /* ── 760px以下：中等压缩（非手机宽屏平板）── */
      #meow-voice-bgm-dock.compact{width:268px !important}
      ...
      #meow-voice-bgm-dock.compact.collapsed{transform:translate3d(calc(100% - 82px),0,0)}
      /* ── 手机（≤640px）：悬浮可拖拽，唱片始终在视口内 ── */
      @media (max-width: 640px){
        #meow-voice-bgm-dock{
          width:270px !important;
          max-width:calc(100vw - 16px) !important;
          /* right/bottom/top/left 由 JS 控制，CSS 不干预 */
        }
        /* 唱片不再溢出到 dock 左侧，改为内嵌定位 */
        #meow-voice-bgm-dock .mv-bgm-shell{padding-left:92px !important;min-height:92px !important}
        #meow-voice-bgm-dock .mv-bgm-disc-wrap{
          left:4px !important;top:6px !important;
          width:84px !important;height:84px !important
        }
        #meow-voice-bgm-dock .mv-bgm-disc{width:74px !important;height:74px !important}
        #meow-voice-bgm-dock .mv-bgm-tonearm{top:-1px;right:-1px;width:46px;height:46px}
        #meow-voice-bgm-dock .mv-bgm-arm-knob{right:5px;top:2px;width:10px;height:10px}
        #meow-voice-bgm-dock .mv-bgm-arm-bar{right:9px;top:10px;width:26px;height:3px}
        #meow-voice-bgm-dock .mv-bgm-arm-head{left:9px;top:28px;width:14px;height:6px}
        #meow-voice-bgm-dock .mv-bgm-panel{padding:10px 9px 9px 0;border-radius:18px}
        #meow-voice-bgm-dock .mv-bgm-name{font-size:13px;max-width:120px}
        #meow-voice-bgm-dock .mv-bgm-sub{font-size:10px;max-width:134px}
        ...
        #meow-voice-bgm-dock .mv-bgm-lyric{min-height:40px;padding:7px 8px;font-size:10px}
        /* 折叠态：不用 transform 推出视口，改为只隐藏面板 */
        #meow-voice-bgm-dock.collapsed{
          transform:none !important;
          width:92px !important;
        }
        #meow-voice-bgm-dock.collapsed .mv-bgm-panel{display:none !important}
        #meow-voice-bgm-dock.collapsed .mv-bgm-close{display:none !important}
        /* 展开态 */
        #meow-voice-bgm-dock:not(.collapsed){width:270px !important}
      }
    ...
    _setupBgmDockDrag(root);
    _applyBgmDockMobilePos(root);
    return root;
  }
  ...
  function _renderBgmDock(cArg) {
    ...
    root.classList.toggle('compact', dockVW > 640 && dockVW <= 760);
    _applyBgmDockMobilePos(root);
    // 展开时确保面板不超出右侧视口
    if (_isBgmMobile() && !root.classList.contains('collapsed')) {
      const vw = W.innerWidth || 375;
      const x = parseFloat(root.style.left) || 0;
      if (x + 270 > vw - 4) root.style.left = Math.max(4, vw - 274) + 'px';
    }
