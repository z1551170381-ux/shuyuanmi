...
#${ID}[data-theme="frost"]{
  ...
  --ph-row-bg: rgba(255,255,255,.42);
  --ph-row-hover: rgba(255,255,255,.58);
  --ph-searchbox-bg: rgba(255,255,255,.54);
  --ph-searchbox-text: rgba(40,48,62,.38);
  --ph-discover-bg: rgba(255,255,255,.34);
  --ph-discover-border: rgba(255,255,255,.44);
  ...
  --ph-dock-surface: rgba(255,255,255,.22);
  --ph-dock-line: rgba(255,255,255,.54);
  --ph-shell-border: rgba(255,255,255,.72);
  --ph-shell-shadow: 0 24px 64px rgba(138,152,176,.14), 0 10px 24px rgba(138,152,176,.08);
}
...
#${ID}[data-theme="frost"] .phWallpaper{
  ...
  background-image:
    var(--ph-wallpaper-url, none),
    radial-gradient(circle at 18% 18%, rgba(255,214,230,.24), transparent 34%),
    radial-gradient(circle at 78% 18%, rgba(206,221,255,.22), transparent 30%),
    radial-gradient(circle at 72% 76%, rgba(226,214,255,.18), transparent 34%),
    radial-gradient(circle at 24% 78%, rgba(255,244,225,.18), transparent 30%),
    linear-gradient(180deg, rgba(255,255,255,.88), rgba(247,245,242,.78));
...
#${ID}[data-theme="frost"] .phShell{
  border-color:var(--ph-shell-border, rgba(255,255,255,.72));
  box-shadow:var(--ph-shell-shadow, 0 24px 64px rgba(138,152,176,.14), 0 10px 24px rgba(138,152,176,.08));
  background:
...
#${ID}[data-theme="frost"] .wxCharSettingsWrap .wxCSHeader,
...
#${ID}[data-theme="frost"] .wxStatePanelCard{
  background:
    ...
    var(--ph-glass-strong) !important;
  border-color:rgba(255,255,255,.74) !important;
  box-shadow:0 10px 26px rgba(126,142,166,.08) !important;
  backdrop-filter:blur(calc(var(--ph-glass-blur) + 2px)) saturate(128%) !important;
  -webkit-backdrop-filter:blur(calc(var(--ph-glass-blur) + 2px)) saturate(128%) !important;
}
...
#${ID}[data-theme="frost"] .wxGroupHeader{
  ...
  box-shadow:0 8px 20px rgba(126,142,166,.06);
  backdrop-filter:blur(calc(var(--ph-glass-blur) + 1px)) saturate(125%);
  -webkit-backdrop-filter:blur(calc(var(--ph-glass-blur) + 1px)) saturate(125%);
}
...
#${ID}[data-theme="frost"] .wxChatPlusGrid{
  ...
  box-shadow:none !important;
  backdrop-filter:blur(calc(var(--ph-glass-blur) + 4px)) saturate(130%) !important;
  -webkit-backdrop-filter:blur(calc(var(--ph-glass-blur) + 4px)) saturate(130%) !important;
}
...
#${ID}[data-theme="frost"] .phAppBody{
  ...
  backdrop-filter:blur(calc(var(--phAppBlur,16px) * .32 + 3px)) saturate(122%);
  -webkit-backdrop-filter:blur(calc(var(--phAppBlur,16px) * .32 + 3px)) saturate(122%);
}
...
#${ID}[data-theme="frost"] .chatBubble.them .cbContent{
  ...
  box-shadow:0 6px 18px rgba(126,142,166,.06);
}
...
#${ID}[data-theme="frost"] .chatSendBtn{
  box-shadow:0 8px 18px rgba(126,142,166,.08);
}
...
// 你后续要加新 app：在这里加一条 + 在 renderApp 的 switch 里加渲染分支（见后文“🎬 renderApp”标注）
      ...
      function _phFlatIcon(emoji){
        ...
        case '⏰': return s('<path d="M22 5.72l-4.6-3.86-1.29 1.53 4.6 3.86L22 5.72zM7.88 3.39L6.6 1.86 2 5.71l1.29 1.53 4.59-3.85zM12.5 8H11v6l4.75 2.85.75-1.23-4-2.37V8zM12 4c-4.97 0-9 4.03-9 9s4.02 9 9 9c4.97 0 9-4.03 9-9s-4.03-9-9-9zm0 16c-3.87 0-7-3.13-7-7s3.13-7 7-7 7 3.13 7 7-3.13 7-7 7z"/>');
...
#${ID}[data-theme="medieval"]{
  --ph-bg-primary: linear-gradient(145deg,#1e1008 0%,#2e1a0c 50%,#3c2210 100%);
  --ph-glass: rgba(210,175,130,.10);
  --ph-glass-strong: rgba(210,175,130,.17);
  --ph-glass-border: rgba(210,175,130,.22);
  --ph-glass-blur: 18px;
  --ph-text: rgba(255,238,210,.92);
  --ph-text-sub: rgba(255,220,180,.58);
  --ph-text-dim: rgba(255,200,150,.32);
  --ph-accent: #d4956a;
  --ph-accent2: #e8b488;
  --ph-accent-grad: linear-gradient(135deg,#c07840,#e0a060);
  --ph-shadow: rgba(0,0,0,.32);
  --ph-sep: rgba(210,175,130,.12);
  --ph-row-bg: rgba(210,175,130,.08);
  --ph-row-hover: rgba(210,175,130,.14);
  --ph-input-bg: rgba(210,175,130,.10);
  --ph-input-border: rgba(210,175,130,.20);
  --ph-send-btn: #c07840;
  --ph-send-icon: #fff;
  --ph-chat-me-bubble: linear-gradient(135deg,#c07840,#e0a060);
  --ph-chat-me-text: #fff;
  --ph-chat-them-bubble: rgba(210,175,130,.14);
  --ph-chat-them-text: rgba(255,238,210,.92);
  --ph-tabbar-bg: rgba(20,10,4,.96);
  --ph-tabbar-border: rgba(210,175,130,.14);
  --ph-tabbar-on: #d4956a;
  --ph-tabbar-off: rgba(255,220,180,.38);
  --ph-appbar-bg: rgba(20,10,4,.96);
  --ph-appbody-bg: rgba(18,9,3,.98);
  --ph-topbar-bg: rgba(20,10,4,.96);
  --ph-topbar-border: rgba(210,175,130,.12);
  --ph-topbar-title: rgba(255,238,210,.90);
  --ph-topbar-btn: rgba(255,220,180,.50);
  --ph-sticker-bg: rgba(18,9,3,.98);
  --ph-input-area-bg: rgba(20,10,4,.97);
  --ph-input-text: rgba(255,238,210,.88);
  --ph-input-ph: rgba(255,200,150,.30);
  --ph-wechat-me-bubble: linear-gradient(135deg,#c07840,#e0a060);
  --ph-wechat-me-text: #fff;
  --ph-wechat-them-bubble: rgba(210,175,130,.14);
  --ph-wechat-them-text: rgba(255,238,210,.90);
  --ph-wechat-bg: rgba(14,7,2,.99);
  --ph-plus-popup-bg: rgba(30,14,6,.97);
  --ph-modal-bg: rgba(30,15,6,.97);
  --ph-modal-text: rgba(255,238,210,.88);
  --ph-confirm-bg: rgba(28,13,5,.99);
  --ph-confirm-text: rgba(255,238,210,.88);
  --ph-confirm-sep: rgba(210,175,130,.12);
  --ph-confirm-cancel: rgba(255,220,180,.40);
  --ph-confirm-danger: #f87171;
  --ph-ico-list: rgba(255,238,210,.88);
  --ph-discover-bg: rgba(210,175,130,.08);
  --ph-discover-border: rgba(210,175,130,.10);
  --ph-discover-name: rgba(255,238,210,.88);
  --ph-discover-arrow: rgba(255,200,150,.28);
  --ph-searchbox-bg: rgba(210,175,130,.10);
  --ph-searchbox-text: rgba(255,220,180,.45);
}

/* ── cyber：赛博朋克 ── */
#${ID}[data-theme="cyber"]{
  --ph-bg-primary: linear-gradient(145deg,#040408 0%,#06080f 50%,#040c18 100%);
  --ph-glass: rgba(0,220,120,.06);
  --ph-glass-strong: rgba(0,220,120,.11);
  --ph-glass-border: rgba(0,220,120,.18);
  --ph-glass-blur: 18px;
  --ph-text: rgba(200,255,230,.94);
  --ph-text-sub: rgba(150,240,200,.60);
  --ph-text-dim: rgba(100,200,160,.35);
  --ph-accent: #00e890;
  --ph-accent2: #00ccff;
  --ph-accent-grad: linear-gradient(135deg,#00e890,#00ccff);
  --ph-shadow: rgba(0,0,0,.50);
  --ph-sep: rgba(0,220,120,.10);
  --ph-row-bg: rgba(0,220,120,.05);
  --ph-row-hover: rgba(0,220,120,.09);
  --ph-input-bg: rgba(0,220,120,.07);
  --ph-input-border: rgba(0,220,120,.18);
  --ph-send-btn: #00e890;
  --ph-send-icon: #000;
  --ph-chat-me-bubble: linear-gradient(135deg,#00e890,#00ccff);
  --ph-chat-me-text: #000;
  --ph-chat-them-bubble: rgba(0,220,120,.10);
  --ph-chat-them-text: rgba(200,255,230,.92);
  --ph-tabbar-bg: rgba(4,4,8,.97);
  --ph-tabbar-border: rgba(0,220,120,.12);
  --ph-tabbar-on: #00e890;
  --ph-tabbar-off: rgba(150,240,200,.35);
  --ph-appbar-bg: rgba(4,4,8,.97);
  --ph-appbody-bg: rgba(4,4,8,.99);
  --ph-topbar-bg: rgba(4,4,8,.97);
  --ph-topbar-border: rgba(0,220,120,.10);
  --ph-topbar-title: rgba(200,255,230,.92);
  --ph-topbar-btn: rgba(150,240,200,.50);
  --ph-sticker-bg: rgba(4,4,8,.99);
  --ph-input-area-bg: rgba(4,4,8,.97);
  --ph-input-text: rgba(200,255,230,.88);
  --ph-input-ph: rgba(100,200,160,.35);
  --ph-wechat-me-bubble: linear-gradient(135deg,#00e890,#00ccff);
  --ph-wechat-me-text: #000;
  --ph-wechat-them-bubble: rgba(0,220,120,.10);
  --ph-wechat-them-text: rgba(200,255,230,.92);
  --ph-wechat-bg: rgba(3,3,6,.99);
  --ph-plus-popup-bg: rgba(8,14,20,.97);
  --ph-modal-bg: rgba(6,10,16,.98);
  --ph-modal-text: rgba(200,255,230,.88);
  --ph-confirm-bg: rgba(6,10,16,.99);
  --ph-confirm-text: rgba(200,255,230,.88);
  --ph-confirm-sep: rgba(0,220,120,.10);
  --ph-confirm-cancel: rgba(150,240,200,.40);
  --ph-confirm-danger: #ff5f5f;
  --ph-ico-list: rgba(200,255,230,.88);
  --ph-discover-bg: rgba(0,220,120,.05);
  --ph-discover-border: rgba(0,220,120,.09);
  --ph-discover-name: rgba(200,255,230,.90);
  --ph-discover-arrow: rgba(100,200,160,.35);
  --ph-searchbox-bg: rgba(0,220,120,.07);
  --ph-searchbox-text: rgba(150,240,200,.42);
}

/* ── sakura：暗紫樱花 ── */
#${ID}[data-theme="sakura"]{
  --ph-bg-primary: linear-gradient(145deg,#180d1c 0%,#22103a 50%,#2e1240 100%);
  --ph-glass: rgba(255,160,200,.08);
  --ph-glass-strong: rgba(255,160,200,.14);
  --ph-glass-border: rgba(255,160,200,.20);
  --ph-glass-blur: 20px;
  --ph-text: rgba(255,230,240,.92);
  --ph-text-sub: rgba(255,200,220,.58);
  --ph-text-dim: rgba(230,170,200,.32);
  --ph-accent: #f472b6;
  --ph-accent2: #c084fc;
  --ph-accent-grad: linear-gradient(135deg,#f472b6,#c084fc);
  --ph-shadow: rgba(0,0,0,.35);
  --ph-sep: rgba(255,160,200,.10);
  --ph-row-bg: rgba(255,160,200,.07);
  --ph-row-hover: rgba(255,160,200,.12);
  --ph-input-bg: rgba(255,160,200,.08);
  --ph-input-border: rgba(255,160,200,.18);
  --ph-send-btn: #f472b6;
  --ph-send-icon: #fff;
  --ph-chat-me-bubble: linear-gradient(135deg,#f472b6,#c084fc);
  --ph-chat-me-text: #fff;
  --ph-chat-them-bubble: rgba(255,160,200,.11);
  --ph-chat-them-text: rgba(255,230,240,.92);
  --ph-tabbar-bg: rgba(18,8,22,.96);
  --ph-tabbar-border: rgba(255,160,200,.12);
  --ph-tabbar-on: #f472b6;
  --ph-tabbar-off: rgba(255,200,220,.38);
  --ph-appbar-bg: rgba(18,8,22,.96);
  --ph-appbody-bg: rgba(16,7,20,.98);
  --ph-topbar-bg: rgba(18,8,22,.96);
  --ph-topbar-border: rgba(255,160,200,.10);
  --ph-topbar-title: rgba(255,230,240,.90);
  --ph-topbar-btn: rgba(255,200,220,.50);
  --ph-sticker-bg: rgba(16,7,20,.97);
  --ph-input-area-bg: rgba(18,8,22,.97);
  --ph-input-text: rgba(255,230,240,.88);
  --ph-input-ph: rgba(230,170,200,.32);
  --ph-wechat-me-bubble: linear-gradient(135deg,#f472b6,#c084fc);
  --ph-wechat-me-text: #fff;
  --ph-wechat-them-bubble: rgba(255,160,200,.12);
  --ph-wechat-them-text: rgba(255,230,240,.92);
  --ph-wechat-bg: rgba(14,6,18,.99);
  --ph-plus-popup-bg: rgba(28,10,36,.97);
  --ph-modal-bg: rgba(26,10,32,.98);
  --ph-modal-text: rgba(255,230,240,.88);
  --ph-confirm-bg: rgba(26,10,32,.99);
  --ph-confirm-text: rgba(255,230,240,.88);
  --ph-confirm-sep: rgba(255,160,200,.10);
  --ph-confirm-cancel: rgba(255,200,220,.40);
  --ph-confirm-danger: #fb7185;
  --ph-ico-list: rgba(255,230,240,.88);
  --ph-discover-bg: rgba(255,160,200,.07);
  --ph-discover-border: rgba(255,160,200,.09);
  --ph-discover-name: rgba(255,230,240,.90);
  --ph-discover-arrow: rgba(230,170,200,.32);
  --ph-searchbox-bg: rgba(255,160,200,.08);
  --ph-searchbox-text: rgba(255,200,220,.42);
}
...
#${ID} .wxDIco:not(.wxDIcoThemed) svg.phIco,
...
#${ID} .phAppIcon[data-app="calendar"] .ai{ background:var(--ph-icon-tint, linear-gradient(135deg,rgba(160,136,181,.75),rgba(189,164,209,.65))); }
#${ID} .phAppIcon[data-app="forum"] .ai{ background:var(--ph-icon-tint, linear-gradient(135deg,rgba(139,157,175,.75),rgba(168,184,200,.65))); }
#${ID} .phAppIcon[data-app="weather"] .ai{ background:var(--ph-icon-tint, linear-gradient(135deg,rgba(196,168,130,.78),rgba(217,196,168,.65))); }
#${ID} .phAppIcon[data-app="themes"] .ai{ background:var(--ph-icon-tint, linear-gradient(135deg,rgba(184,169,201,.75),rgba(209,196,224,.65))); }
#${ID} .phAppIcon .at{
...
#${ID} .phDockBtn[data-app="sms"] .di{ background:var(--ph-icon-tint, linear-gradient(135deg,rgba(141,168,184,.75),rgba(168,192,207,.65))); }
#${ID} .phDockBtn[data-app="browser"] .di{ background:var(--ph-icon-tint, linear-gradient(135deg,rgba(123,158,168,.75),rgba(154,184,194,.65))); }
#${ID} .phDockBtn[data-app="photos"] .di{ background:var(--ph-icon-tint, linear-gradient(135deg,rgba(164,196,154,.75),rgba(188,216,180,.65))); }
#${ID} .phDockBtn[data-app="settings"] .di{ background:var(--ph-icon-tint, linear-gradient(135deg,rgba(158,158,175,.72),rgba(184,184,200,.60))); }
...
#${ID} .phAppSubTitle{
  ...
  font-size:10.5px; color:var(--ph-text); opacity:.52; font-weight:400;
  white-space:nowrap; letter-spacing:.15px; margin-top:2px;
}
...
#${ID} .wxChatSendBtn{
  ...
  background:var(--ph-accent, var(--ph-accent, #07c160)); color:#fff; font-size:13px; font-weight:600;
  transition:opacity .1s; box-shadow:0 2px 6px rgba(0,0,0,.10);
}
...
#${ID} .wxEditMsgBox textarea:focus{ border-color:var(--ph-accent, var(--ph-accent, #07c160)); }
...
#${ID} .wxEditMsgBox .wxEMBtn.ok{
  background:var(--ph-accent, var(--ph-accent, #07c160)); color:#fff; font-weight:600;
}
...
#${ID} .wxStickerTabs .wxStkTab.on{ color:var(--ph-accent, #07c160); border-bottom-color:var(--ph-accent, #07c160); font-weight:600; }
...
#${ID} .wxCharSettingsWrap .wxCSItem{
  ...
  border-bottom:1px solid var(--ph-sep); cursor:pointer; transition:background .12s;
  background:transparent;
}
...
#${ID} .wxReminderToggle.on{ background:var(--ph-accent, var(--ph-accent, #07c160)); }
...
/* Theme cards */
#${ID} .themeGrid{
  display:grid; grid-template-columns:repeat(3,1fr); gap:10px; padding:12px 14px;
}
#${ID} .themeCard{
  appearance:none; border:1.5px solid var(--ph-sep); cursor:pointer;
  border-radius:var(--ph-radius-md); padding:10px 6px 8px; text-align:center;
  background:var(--ph-glass); transition:all .15s; font-size:11px; font-weight:500;
  color:var(--ph-text-sub);
}
#${ID} .themeCard:hover{ border-color:var(--ph-glass-border); transform:translateY(-1px); }
#${ID} .themeCard .tcPreview{
  width:100%; height:44px; border-radius:10px; margin-bottom:8px;
        ...
        function _phApplyCustomAvatars(){
          ...
          root.querySelectorAll(sel).forEach(el=>{
            ...
            if (el.classList.contains('pwAvatar') || el.classList.contains('wxMeAvatar')) key = 'me';
              ...
              if (!key){ const nm = el.parentElement?.querySelector('.momentName,.feedName,.npcProfileName'); if (nm) key = (nm.textContent||'').trim(); }
            }
            const imgSrc = key ? (avatars[key]||'') : '';
            const existingImg = el.querySelector('img.phAvImg');
            if (imgSrc){
              if (existingImg){
                if (existingImg.getAttribute('data-avkey') !== key){ existingImg.src = imgSrc; existingImg.setAttribute('data-avkey', key); }
              } else {
                el.style.backgroundImage = ''; el.style.backgroundSize = ''; el.style.backgroundPosition = ''; el.style.color = ''; el.style.fontSize = '';
                const img = document.createElement('img');
                img.className = 'phAvImg';
                img.setAttribute('data-avkey', key);
                img.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;object-fit:cover;border-radius:inherit;pointer-events:none;display:block;';
                img.src = imgSrc;
            ...
            <div class="phGrid">
              <div class="pw pwProfile" style="grid-column:1/span 4;grid-row:1/span 2;" data-item="profile">
                ...
                <div class="pwProfileRow">
                  <div class="pwAvatar">${phoneGetAvatar('me')?`<img src="${phoneGetAvatar('me')}" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;"/>`:'👤'}</div>
                  <div>
          ...
          if (act === 'wxCHDetailRegen'){
            ...
            overlay.innerHTML = `<div class="wxConfirmBox">
              ...
              <div class="wxCBtns">
                ...
                <button class="wxCBtn" data-act="wxCHRegenOk" data-chnpcid="${esc(nid)}" style="color:var(--ph-accent, #07c160);font-weight:600;">确定</button>
              </div>
            </div>`;
      ...
      function _openAddFriendDialog(){
        ...
        overlay.innerHTML = `<div class="wxConfirmBox" style="min-width:260px;text-align:left;">
          ...
          <div style="display:flex;gap:8px;">
            ...
            <button data-act="wxAddFriendConfirm" style="flex:1;padding:10px;border-radius:8px;border:0;background:var(--ph-accent, var(--ph-accent, #07c160));color:#fff;font-size:13px;font-weight:600;cursor:pointer;">添加</button>
          </div>
        </div>`;
      ...
      function _openCreateGroupDialog(){
        ...
        overlay.innerHTML = `<div class="wxConfirmBox" style="min-width:260px;text-align:left;">
          ...
          <div style="display:flex;gap:8px;">
            ...
            <button data-act="wxCreateGroupConfirm" style="flex:1;padding:10px;border-radius:8px;border:0;background:var(--ph-accent, var(--ph-accent, #07c160));color:#fff;font-size:13px;font-weight:600;cursor:pointer;">创建</button>
          </div>
        </div>`;
  ...
  return '<span style="font-size:14px;">'+icon+'</span>';
          ...
          for (const g of groups){
            ...
            for (const c of members){
              html += `<div class="wxContactItem" data-chatid="${esc(c.id)}">
                <div class="wxCIAvatar">${(()=>{const _ci=phoneGetAvatar(c.id||'');return _ci?`<img src="${_ci}" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;"/>`:esc((c.avatar||c.name||'?').charAt(0));})()}</div>
      ...
      function _renderWBImportPageWithData(parentContainer, chars){
        ...
        if (!chars.length){
          ...
          html += '<div style="padding:0 14px;"><button data-act="c1WBImportAll" style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(0,0,0,.08);background:rgba(255,255,255,.82);color:var(--ph-accent, #07c160);font-size:13px;font-weight:500;cursor:pointer;margin-bottom:8px;">全部导入为候选</button></div>';
          for (var i = 0; i < chars.length; i++){
            ...
            html += statusText ? '<span style="font-size:11px;color:rgba(20,24,28,.4);">' + statusText + '</span>' : '<button data-act="c1WBImportOne" data-cidx="' + i + '" style="border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:11px;padding:4px 10px;border-radius:8px;cursor:pointer;">导入</button>';
            html += '</div>';
      ...
      function _renderSTCardImportPage(parentContainer){
        ...
        if (!result || !result.available || !result.characters.length){
          ...
          for (var i = 0; i < result.characters.length; i++){
            ...
            html += '<span style="font-size:14px;font-weight:600;color:rgba(20,24,28,.85);">🎭 ' + esc(c.name) + '</span>';
            html += statusText ? '<span style="font-size:11px;color:rgba(20,24,28,.4);">' + statusText + '</span>' : '<button data-act="c1STImportOne" data-cidx="' + i + '" style="border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:11px;padding:4px 10px;border-radius:8px;cursor:pointer;">导入</button>';
            html += '</div>';
      ...
      function _openGroupManager(parentContainer){
        ...
        overlay.innerHTML = `<div class="wxConfirmBox" style="min-width:260px;max-width:300px;width:calc(100% - 40px);text-align:left;box-sizing:border-box;overflow:visible;">
          ...
          <div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:nowrap;min-width:0;">
            ...
            <button data-gact="add" style="padding:6px 12px;border:0;border-radius:6px;background:var(--ph-accent, #07c160);color:#fff;font-size:12px;cursor:pointer;flex-shrink:0;white-space:nowrap;">添加</button>
          </div>
      ...
      function renderChatHistoryPage(container){
        ...
        let html = `<div style="padding:0;">
          <div style="display:flex;border-bottom:1px solid rgba(0,0,0,.06);background:rgba(255,255,255,.5);">
            <button data-act="wxCHTabSwitch" data-chtab="summary" style="flex:1;padding:10px;border:0;background:transparent;font-size:13px;font-weight:${activeTab==='summary'?'600':'400'};color:${activeTab==='summary'?'var(--ph-accent, #07c160)':'rgba(20,24,28,.5)'};cursor:pointer;border-bottom:2px solid ${activeTab==='summary'?'var(--ph-accent, #07c160)':'transparent'};">聊天总结</button>
            <button data-act="wxCHTabSwitch" data-chtab="moments" style="flex:1;padding:10px;border:0;background:transparent;font-size:13px;font-weight:${activeTab==='moments'?'600':'400'};color:${activeTab==='moments'?'var(--ph-accent, #07c160)':'rgba(20,24,28,.5)'};cursor:pointer;border-bottom:2px solid ${activeTab==='moments'?'var(--ph-accent, #07c160)':'transparent'};">朋友圈评论</button>
          </div>`;
          ...
          if (!threadList.length){
            ...
            threadList.forEach(thread => {
              ...
              const statusHtml = hasSummary
                ? `<span style="color:var(--ph-accent, #07c160);">✅ 已总结 · ${_timeAgo(sumData.updatedAt)}</span>`
      ...
      function renderSummarySettingsPage(){
        ...
        let html = `<div style="padding:14px;">
          ...
          <button data-act="wxSumSettingsSave" style="width:100%;padding:12px;border-radius:10px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:14px;font-weight:600;cursor:pointer;">💾 保存设置</button>
        </div>`;
        body.innerHTML = html;
      }
      ...
      function renderGlobalWBPage(container){
        ...
        let html = `<div style="padding:14px;">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-top:12px;padding:10px 12px;background:rgba(255,255,255,.7);border-radius:10px;border:1px solid rgba(0,0,0,.06);">
            ...
            <button data-act="gwbToggle" class="wxToggleBtn ${data.enabled?'on':'off'}" style="width:44px;height:26px;border-radius:13px;border:0;cursor:pointer;position:relative;transition:background .2s;background:${data.enabled?'var(--ph-accent, #07c160)':'rgba(0,0,0,.15)'};">
          ...
          <div style="margin-top:14px;padding:12px 14px;background:rgba(255,255,255,.85);border-radius:12px;border:1px solid rgba(0,0,0,.07);">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
              ...
              <button data-act="gwbTavernCtxToggle" style="width:44px;height:26px;border-radius:13px;border:0;cursor:pointer;position:relative;transition:background .2s;background:${data.tavernCtxEnabled?'var(--ph-accent, #07c160)':'rgba(0,0,0,.15)'};">
          ...
          <button data-act="gwbSave" style="width:100%;margin-top:12px;padding:12px;border-radius:10px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:14px;font-weight:600;cursor:pointer;">保存</button>
        ...
        container.querySelector('[data-act="gwbToggle"]')?.addEventListener('click', function(){
          ...
          this.style.background = data.enabled ? 'var(--ph-accent, #07c160)' : 'rgba(0,0,0,.15)';
        ...
        container.querySelector('[data-act="gwbTavernCtxToggle"]')?.addEventListener('click', function(){
          ...
          this.style.background = data.tavernCtxEnabled ? 'var(--ph-accent, #07c160)' : 'rgba(0,0,0,.15)';
          const dot = this.querySelector('span');
      ...
      function renderVoiceApiPage(container){
        ...
        container.querySelector('[data-act="vaAutoVoiceToggle"]')?.addEventListener('click', function(){
          ...
          this.style.background = data.autoVoice ? 'var(--ph-accent, #07c160)' : 'rgba(0,0,0,.15)';
          const dot = this.querySelector('span');
      ...
      function renderRelationPage(container){
        ...
        html += `<div style="display:flex;gap:8px;margin-top:12px;">
            ...
            <button data-act="relSave" style="flex:1;padding:10px;border-radius:10px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">保存</button>
          </div>
        </div>`;
        container.innerHTML = html;
        ...
        if (blacklist.length){
          blacklist.forEach((b, idx) => {
            html += `<div style="display:flex;align-items:center;padding:10px 12px;margin-bottom:6px;background:rgba(255,255,255,.7);border-radius:10px;border:1px solid rgba(0,0,0,.06);">
              ...
              <button data-act="blRemove" data-blidx="${idx}" style="padding:4px 12px;border-radius:8px;border:1px solid rgba(0,0,0,.08);background:#fff;font-size:11px;color:var(--ph-accent, #07c160);cursor:pointer;">移除</button>
            </div>`;
          });
        } else {
      ...
      function renderMePage(container){
        ...
        html += `<div class="wxMeProfile" data-act="wxMeProfile">
          <div class="wxMeAvatar">${phoneGetAvatar('me')?`<img src="${phoneGetAvatar('me')}" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;"/>`:'😊'}</div>
          <div class="wxMeInfo">
      ...
      function _openFavEditor(container, type, editIdx){
        ...
        overlay.innerHTML = `<div class="wxConfirmBox" style="min-width:280px;text-align:left;max-height:80vh;overflow-y:auto;">
          ...
          <div style="display:flex;gap:8px;">
            ...
            <button data-act="favEditorSave" style="flex:1;padding:10px;border-radius:8px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">保存</button>
          </div>
        </div>`;
        root.appendChild(overlay);
        overlay.addEventListener('click',(e)=>{ if (e.target===overlay) overlay.remove(); });
        ...
        html += `<div style="margin:0 14px 10px;padding:12px 14px;border-radius:12px;background:var(--ph-glass-strong);border:1px solid ${isFollowChatId ? 'var(--ph-accent)' : 'var(--ph-glass-border)'};">
          <div style="display:flex;align-items:center;justify-content:space-between;">
            ...
            <button data-act="${isFollowChatId ? 'personaUnlinkChat' : 'personaFollowChat'}"
              style="border:0;background:${isFollowChatId ? 'rgba(0,0,0,.06)' : 'var(--ph-accent, #07c160)'};color:${isFollowChatId ? 'rgba(20,24,28,.5)' : '#fff'};font-size:11px;padding:5px 10px;border-radius:6px;cursor:pointer;white-space:nowrap;">
      ...
      function _openPersonaEditor(container, editIdx){
        ...
        overlay.innerHTML = `<div class="wxConfirmBox" style="min-width:280px;text-align:left;max-height:80vh;overflow-y:auto;">
          ...
          <div style="display:flex;gap:8px;">
            ...
            <button data-act="pSave" style="flex:1;padding:10px;border-radius:8px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">保存</button>
          </div>
        </div>`;
        root.appendChild(overlay);
        overlay.addEventListener('click',(e)=>{ if(e.target===overlay) overlay.remove(); });
          ...
          function _render(){
            ...
            let html = `<div class="settingSubPage" style="padding:14px;">
              ...
              <div style="display:flex;gap:8px;margin-top:16px;">
                ...
                <button data-act="apiSavePreset" style="flex:1;padding:12px;border-radius:12px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">保存</button>
              </div>
          ...
          function _bindEditorEvents(ct2, pidx){
            ...
            ct2.querySelector('[data-act="apiTestConn"]')?.addEventListener('click', async ()=>{
              ...
              try{
                ...
                if (resp.ok){
                  resultEl.innerHTML = `<div style="font-size:12px;color:var(--ph-accent, #07c160);font-weight:600;">✅ 连接成功（${ms}ms）</div>`;
                } else {
        ...
        function _apiExportDialog(singleMode, pidx){
          ...
          overlay.innerHTML = `<div class="wxConfirmBox" style="min-width:280px;text-align:left;">
            ...
            <label style="display:flex;align-items:center;gap:8px;padding:8px 0;cursor:pointer;">
              <input type="checkbox" data-el="incKey" style="width:16px;height:16px;accent-color:var(--ph-accent, #07c160);" />
            ...
            <div style="display:flex;gap:8px;margin-top:10px;">
              ...
              <button data-act="exConfirm" style="flex:1;padding:10px;border-radius:8px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">导出</button>
            </div>
          </div>`;
          root.appendChild(overlay);
          overlay.addEventListener('click',(e)=>{ if(e.target===overlay) overlay.remove(); });
      ...
      function _dmOpenExportSelector(){
        ...
        _DM_MODULES.forEach((mod,i)=>{
          ...
          checkHtml += `<label style="display:flex;align-items:center;gap:8px;padding:6px 0;cursor:pointer;">
            <input type="checkbox" value="${mod.key}" checked style="width:16px;height:16px;accent-color:var(--ph-accent, #07c160);" />
        ...
        overlay.innerHTML = `<div class="wxConfirmBox" style="min-width:280px;text-align:left;max-height:80vh;overflow-y:auto;">
          ...
          <label style="display:flex;align-items:center;gap:8px;padding:6px 0;cursor:pointer;border-bottom:1px solid rgba(0,0,0,.06);margin-bottom:8px;">
            <input type="checkbox" data-el="selAll" checked style="width:16px;height:16px;accent-color:var(--ph-accent, #07c160);"/>
        ...
        overlay.innerHTML = `<div class="wxConfirmBox" style="min-width:280px;text-align:left;max-height:80vh;">
          ...
          <div style="margin-top:10px;">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
              <input type="radio" name="imMode" value="overwrite" checked style="accent-color:var(--ph-accent, #07c160);"/>
              <span style="font-size:12px;color:var(--ph-text);">覆盖同 key（默认）</span>
            </label>
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-top:4px;">
              <input type="radio" name="imMode" value="merge" style="accent-color:var(--ph-accent, #07c160);"/>
          ...
          <div style="display:flex;gap:8px;margin-top:12px;">
            ...
            <button data-act="imConfirm" style="flex:1;padding:10px;border-radius:8px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">导入</button>
          </div>
        </div>`;
        root.appendChild(overlay);
        overlay.addEventListener('click',(e)=>{ if(e.target===overlay) overlay.remove(); });
      ...
      function _showCopyBox(text){
        ...
        overlay.innerHTML = `<div class="wxConfirmBox" style="min-width:280px;text-align:left;max-height:80vh;">
          ...
          <div style="display:flex;gap:8px;margin-top:10px;">
            ...
            <button data-act="cbCopy" style="flex:1;padding:10px;border-radius:8px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">全选复制</button>
          </div>
        </div>`;
        root.appendChild(overlay);
        overlay.addEventListener('click',(e)=>{ if(e.target===overlay) overlay.remove(); });
      ...
      function _openExportSelector(container){
        ...
        contacts.forEach(c=>{
          checkHtml += `<label style="display:flex;align-items:center;gap:8px;padding:5px 0;cursor:pointer;">
            <input type="checkbox" value="${esc(c.id)}" data-type="contact" checked style="width:16px;height:16px;accent-color:var(--ph-accent, #07c160);"/>
        ...
        Object.keys(groups||{}).forEach(gid=>{
          const g = groups[gid];
          checkHtml += `<label style="display:flex;align-items:center;gap:8px;padding:5px 0;cursor:pointer;">
            <input type="checkbox" value="${esc(gid)}" data-type="group" checked style="width:16px;height:16px;accent-color:var(--ph-accent, #07c160);"/>
        ...
        if (txns.length){
          for (const tx of txns){
            ...
            html += `<div style="padding:8px 14px;display:flex;align-items:center;border-bottom:1px solid rgba(0,0,0,.04);">
              ...
              <div style="font-size:13px;font-weight:600;color:${isSpend?'#e74c3c':'var(--ph-accent, #07c160)'};">${isSpend?'-':'+'}${tx.amount}</div>
            </div>`;
          }
        } else {
        ...
        data.posts.forEach(m=>{
          ...
          html += `<div class="momentItem" data-mid="${esc(m.id)}">
            <div class="momentHead">
              <div class="momentAvatar" data-npcid="${esc(m.authorId||m.author||'')}">${(()=>{const _mi=phoneGetAvatar(m.authorId||m.author||'');return _mi?`<img src="${_mi}" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;"/>`:esc(m.avatar||'👤');})()}</div>
          ...
          html += `<div style="display:flex;align-items:center;justify-content:space-between;margin-top:6px;">
            ...
            <div class="momentCmtInput" data-cmtbox="${esc(m.id)}" style="display:none;">
              ...
              <button data-cmtsend="${esc(m.id)}" style="appearance:none;border:none;background:var(--ph-accent, #07c160);color:#fff;padding:6px 12px;border-radius:14px;font-size:12px;cursor:pointer;font-weight:600;">发送</button>
            </div>
          </div>`;
        });
        html += '</div>';
      ...
      function _openMomentsCoverSettings(){
        ...
        overlay.innerHTML = `<div class="wxConfirmBox" style="min-width:260px;text-align:left;">
          ...
          <div style="display:flex;gap:8px;">
            ...
            <button data-act="mcsSaveBtn" style="flex:1;padding:10px;border-radius:8px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">保存</button>
          </div>
        </div>`;
        root.appendChild(overlay);
        overlay.addEventListener('click',(e)=>{
...
const npc = _wxGetChatTargetMeta(npcId);
      ...
      function _renderCharSettingsPage(contactId){
        ...
        let html = `<div class="wxCharSettingsWrap">
          ...
          <div class="wxCSGroup">
            ...
            <div class="wxCSItem" data-act="wxCSNav" data-cspage="charBehavior" data-npcid="${esc(contactId)}">
              <div class="wxCSIco">${_phFlatIcon('🎭')}</div>
      ...
      function _renderCharBehaviorPage(contactId){
        ...
        function sliderRow(label, key, val){
          return `<div style="padding:12px 14px;border-bottom:1px solid rgba(0,0,0,.05);">
            <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
              <span style="font-size:13px;color:rgba(20,24,28,.8);">${label}</span>
            ...
            <input type="range" min="0" max="100" value="${Math.round(val*100)}" data-sl="${key}"
              style="width:100%;accent-color:var(--ph-accent, #07c160);cursor:pointer;"/>
          </div>`;
        }
        function selectRow(label, key, opts, labels, cur){
          var optsHtml = opts.map(function(o,i){ return `<option value="${o}"${o===cur?' selected':''}>${labels[i]}</option>`; }).join('');
          return `<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid rgba(0,0,0,.05);">
            <span style="font-size:13px;color:rgba(20,24,28,.8);">${label}</span>
            <select data-sel="${key}" style="font-size:12px;border:1px solid rgba(0,0,0,.1);border-radius:8px;padding:4px 8px;background:rgba(255,255,255,.9);outline:none;max-width:140px;">${optsHtml}</select>
          </div>`;
        }
        ...
        var html = `<div style="padding-bottom:20px;">
          ...
          <div style="background:rgba(255,255,255,.88);border-radius:12px;border:1px solid rgba(0,0,0,.07);overflow:hidden;margin:0 14px;">
            ...
            ${selectRow('被冒犯时', 'defenseStyle', DEFENSE_STYLES, DEFENSE_STYLES, b.defenseStyle||'解释')}
      ...
      function _renderCharProfileEdit(contactId){
        ...
        let html = `<div style="padding:14px;">
          ...
          <div style="margin-top:14px;display:flex;gap:8px;">
            <button data-act="wxCSSaveProfile" data-npcid="${esc(contactId)}" style="flex:1;padding:12px;border-radius:12px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:14px;font-weight:600;cursor:pointer;">保存</button>
      ...
      function _wxReminderCustomAdd(npcId){
        const innerHtml = `
          ...
          <div style="display:flex;gap:8px;margin-top:14px;">
            <button data-el="rSave" style="flex:1;padding:10px;border-radius:10px;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">添加</button>
        ...
        ov.querySelectorAll('.rIcoPick').forEach(el=>{
          el.addEventListener('click', ()=>{
            ...
            el.style.borderColor = 'var(--ph-accent, #07c160)';
        ...
        if (mt === 'image'){
          ...
          contentHtml = `<div class="wxCBContent wxCBSpecial" style="padding:0;overflow:hidden;border-radius:12px;min-width:180px;max-width:240px;">
            <div style="display:flex;align-items:center;gap:8px;padding:10px 12px;background:rgba(255,255,255,.9);">
              <button data-ttsplay="${waveId}" style="width:32px;height:32px;border-radius:50%;border:0;background:var(--ph-accent, #07c160);color:#fff;font-size:15px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;">▶</button>
      ...
      function buildSystemPrompt(npcId){
        ...
        // === 0. [角色实时状态] B3：最高优先级，放最前面 ===
        try{
          var stateBlock = _buildStatePromptBlock(npcId);
          if (stateBlock) parts.push(stateBlock);
        }catch(e){}

        // === 0.5 [行为风格] C：角色行为参数 ===
        try{
          var behaviorBlock = _buildBehaviorPromptBlock(npcId);
      ...
      function _hideTypingInAppBar(){
        try{
          var wrap = root.querySelector('[data-ph="appTitleWrap"]');
          var sub  = root.querySelector('[data-ph="appSubTitle"]');
          if (sub) sub.classList.remove('show');
          if (wrap) wrap.classList.remove('hasSubTitle');
        }catch(e){}
      }
      ...
      // ========== 【模块 C】角色行为参数 ==========
      var DEFENSE_STYLES  = ['沉默','反讽','转移话题','解释','撒娇','激烈反驳'];
      var COMFORT_STYLES  = ['讲道理','共情抱抱','转移注意力','给解决方案','冷处理'];
      var PROACTIVE_OPTS  = ['daily','weekly','rare'];
      var PROACTIVE_LABELS= ['每天主动分享','每周偶尔','几乎不主动'];
      var EMOJI_OPTS      = ['rare','normal','many'];
      ...
      function _saveCharBehavior(npcId, b){
        _phSave('charbehavior_'+String(npcId), b);
      }

      // 将行为参数转为注入 system prompt 的短文本
      function _buildBehaviorPromptBlock(npcId){
        try{
          var b = _loadCharBehavior(npcId);
          var lines = [];
          if (b.shortSentenceRatio >= 0.7) lines.push('说话习惯：爱发短句，一次只说一两个字或一小句');
          else if (b.shortSentenceRatio <= 0.3) lines.push('说话习惯：句子偏长，表达完整，不爱碎碎念');
          if (b.burstProbability >= 0.6) lines.push('发消息风格：经常连发多条，想到什么说什么');
          else if (b.burstProbability <= 0.2) lines.push('发消息风格：一次说完，很少连发');
          if (b.emotionExpressive >= 0.7) lines.push('情绪表达：情绪很外放，容易感叹、激动或直接发怒');
          else if (b.emotionExpressive <= 0.3) lines.push('情绪表达：情绪内敛，不容易表现出来，平时看起来很稳');
          lines.push('被误解/被冒犯时：' + b.defenseStyle);
        ...
        try{
          ...
          if (charStateNow && charStateNow.silentUntil > Date.now()){
            ...
            setTimeout(function(){
              ...
              var breakLines = ['……', '缓了一下', '刚才有点上头', '好了冷静一下', '……回来了'];
              var breakMsg = breakLines[Math.floor(Math.random() * breakLines.length)];
              var msgsE = root.querySelector('[data-ph="chatMsgs"]');
              if (msgsE){
                pushLog(npcId, 'them', breakMsg);
                bumpThread(npcId, { lastMsg:breakMsg, lastTime:_now(), unread:0 });
                _wxAppendBubble(msgsE, npc, 'them', breakMsg, _now());
                requestAnimationFrame(function(){ msgsE.scrollTop = msgsE.scrollHeight; });
              }
            }, leftMs + _randomBetween(500, 1500));
            return; // 沉默期内不触发正常 AI 回复
          }
        ...
        const ov = _cpShowOverlay(`
          <div style="text-align:center;">
            ...
            <div style="display:flex;gap:10px;justify-content:center;">
              <button data-act="cpCallStart" data-calltype="voice" style="flex:1;padding:10px;border-radius:12px;border:1px solid rgba(0,0,0,.08);background:var(--ph-accent, #07c160);color:#fff;font-size:13px;cursor:pointer;">📞 语音通话</button>
              <button data-act="cpCallStart" data-calltype="video" style="flex:1;padding:10px;border-radius:12px;border:1px solid rgba(0,0,0,.08);background:var(--ph-accent, #07c160);color:#fff;font-size:13px;cursor:pointer;">📹 视频通话</button>
            </div>
          </div>
        `);
      ...
      function renderForumMe(container){
        ...
        var html = '<div class="forumMePage">' +
          '<div class="forumMeBanner">' +
            '<div class="forumMeAvatar">'+(phoneGetAvatar('me')?'<img src="'+phoneGetAvatar('me')+'" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;"/>':esc(myAvatar))+'</div>' +
      ...
      function openNpcProfile(npcId){
        ...
        let html = `
          <div class="npcProfile">
            ...
            <div class="npcProfileAvatar">${(()=>{const _ni=phoneGetAvatar(npc?.id||'');return _ni?`<img src="${_ni}" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;"/>`:esc(avatar);})()}</div>
  ...
  container.querySelectorAll('input[data-uiapp]').forEach(inp=>{
    inp.addEventListener('input', ()=>{
      ...
      function renderThemes(container){
        ...
        const current = root.getAttribute('data-theme') || 'frost';
        ...
        themes.forEach(t=>{
          html += `<button class="themeCard${t.id===current?' active':''}" data-act="setTheme" data-theme="${t.id}">
            ...
            <span style="color:${t.previewText||'var(--ph-text-sub)'}">${t.name}</span>
          </button>`;
        });
        html += '</div>';
        container.innerHTML = html;
      ...
      function applyTheme(themeId){
        ...
        // UI：始终挂 data-theme（frost 也显式挂，CSS 变量才能生效）
        root.setAttribute('data-theme', themeId || 'frost');
      ...
      function loadTheme(){
        try{
          ...
          // 应用到 UI：没有保存值则默认 frost（空 data-theme = frost）
          if (t && t !== 'frost'){
            root.setAttribute('data-theme', t);
          } else {
            // frost 或未设置：不挂 data-theme 属性（CSS 里 [data-theme="frost"] 匹配）
            // 但为了让 frost 的 CSS 变量生效，需要显式挂 frost
            root.setAttribute('data-theme', 'frost');
          }
        }catch(e){}
      }
