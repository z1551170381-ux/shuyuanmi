...
#${ID} button[data-act="settingsBack"],
#${ID} button[data-act="apiEditorBack"]{
...
function phoneApplyVisualFromSettings(cfg){
  try{
    ...
    root.style.setProperty('--phSurfaceBlur', `${Math.max(0, Math.min(56, Math.round(curBlurPx * 1.35 + 2)))}px`);
    root.style.setProperty('--phAppBodyBlur', `${Math.max(0, Math.min(56, Math.round(appBlurPx * 1.22 + 4)))}px`);
    root.style.setProperty('--ph-appbody-bg', _rgbaVar(appRgb, Math.max(0, appSolidA)));

    if (themeId === 'frost'){
      const softA = isHomeView ? (curA + 0.06) : curA;
      const strongA = isHomeView ? (curStrongA + 0.06) : curStrongA;
      const lineA = isHomeView ? (curBorderA + 0.04) : curBorderA;
      root.style.setProperty('--ph-card-soft', _rgbaVar(curRgb, softA));
      root.style.setProperty('--ph-card-strong', _rgbaVar(curRgb, strongA));
      root.style.setProperty('--ph-card-line', _rgbaVar('255,255,255', lineA));
      root.style.setProperty('--ph-dock-surface', _rgbaVar(curRgb, isHomeView ? (curA + 0.02) : (curA * 0.82 + 0.04)));
      root.style.setProperty('--ph-dock-line', _rgbaVar('255,255,255', lineA));
      root.style.setProperty('--ph-app-icon-bg', `linear-gradient(180deg, ${_rgbaVar(curRgb, isHomeView ? (curStrongA + 0.10) : (curStrongA + 0.04))}, ${_rgbaVar(curRgb, isHomeView ? (curA + 0.10) : (curA + 0.04))})`);
      root.style.setProperty('--ph-app-icon-border', _rgbaVar('255,255,255', lineA + 0.04));
...
#${ID} .phAppIcon{
  ...
  gap:7px; padding:14px 0 10px; transition:transform .12s, background .12s;
...
#${ID} .phAppIcon .ai{
  ...
  margin-top:8px;
  display:flex; align-items:center; justify-content:center;
  font-size:17px; line-height:1; color:var(--ph-app-icon-fg); flex-shrink:0;
  background:var(--ph-accent-grad);
}
...
#${ID} .phAppIcon .at{
  ...
  min-height:16px; text-align:center; padding:0 4px 2px; flex-shrink:0;
}
...
#${ID} .settingSubPage[data-setting-page="uiHome"] .phCard{
  background:rgba(var(--phHomeRGB),var(--phHomeA)) !important;
  border-color:rgba(var(--phHomeRGB),var(--phHomeBorderA)) !important;
  backdrop-filter:blur(var(--phHomeBlur)) !important;
  -webkit-backdrop-filter:blur(var(--phHomeBlur)) !important;
}
#${ID} .settingSubPage[data-setting-page="uiApp"]{
  background:rgba(var(--phAppSolidRGB),var(--phAppSolidA)) !important;
  backdrop-filter:blur(var(--phAppBodyBlur, var(--phAppBlur))) !important;
  -webkit-backdrop-filter:blur(var(--phAppBodyBlur, var(--phAppBlur))) !important;
  border-radius:18px;
}
#${ID} .settingSubPage[data-setting-page="uiApp"] .phCard{
  background:rgba(var(--phAppRGB),var(--phAppA)) !important;
  border-color:rgba(var(--phAppRGB),var(--phAppBorderA)) !important;
  backdrop-filter:blur(var(--phAppBlur)) !important;
  -webkit-backdrop-filter:blur(var(--phAppBlur)) !important;
}
...
const npc = _wxGetChatTargetMeta(npcId);
      ...
      function renderSettings(container){
        ...
        const _secOrder = { '高级':0, '外观':1, '功能':2, '关于':3, '危险区域':4 };
...
function renderSettingsUIHome(container){
  ...
  container.innerHTML = `
    <div class="settingSubPage" data-setting-page="uiHome">
      <button data-act="settingsBack" style="appearance:none;border:0;background:var(--ph-glass);
        color:var(--ph-text-sub);padding:6px 12px;border-radius:12px;cursor:pointer;font-size:12px;
        margin-bottom:12px;">‹ 返回设置</button>
...
function renderSettingsUIApp(container){
  ...
  container.innerHTML = `
    <div class="settingSubPage" data-setting-page="uiApp">
      <button data-act="settingsBack" style="appearance:none;border:0;background:var(--ph-glass);
        color:var(--ph-text-sub);padding:6px 12px;border-radius:12px;cursor:pointer;font-size:12px;
        margin-bottom:12px;">‹ 返回设置</button>
